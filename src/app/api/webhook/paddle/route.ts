...
/**
 * POST /api/webhook/paddle — Paddle (Merchant-of-Record) webhook. Paddle signs
 * the RAW body, so we must read text() before verifying (never req.json()).
 * Grip is granted only via settle(), same contracts as the Midtrans path.
 */
export async function POST(req: Request) {
  const gateway = getWebhookGateway();
  if (!gateway) {
    // This deployment is not configured for Paddle.
